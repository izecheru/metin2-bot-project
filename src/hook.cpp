...
/// <returns>True or false if we got the IDirect3D9 pointer</returns>
...
LRESULT WINAPI Hook::WndProc(HWND hWnd, UINT uMsg, WPARAM wParam,
  LPARAM lParam)
{
  if (UserInterface::getInstance()->isMenuVisible())
  {
    ...
    if (uMsg == WM_KEYDOWN)
    {
      ...
      {
        case VK_INSERT:
          UserInterface::getInstance()->flipMenu();
          break;
        case VK_END:
          UserInterface::getInstance()->flipDetach();
          break;
        default:
          break;
      }
...
{
  if (UserInterface::getInstance()->isDetached())
  {
  ...
  if (UserInterface::getInstance()->isMenuVisible())
  {
    UserInterface::getInstance()->renderMenu();
  }

  // a special window for my character coords
  if (UserInterface::getInstance()->isCoordinatesWinVisible())
  {
    UserInterface::getInstance()->renderCoordinatesWin();
  }

...
public:
  ...
  string(const char *_Ptr)
      : _Alval(allocator<char>::template rebind<char>::other()) {
    _Tidy();
  ...
  const char *c_str() const { return (_Myptr()); }
  ...
  void _Copy(allocator<char>::size_type _Newsize,
             allocator<char>::size_type _Oldlen) {
...
template <class T, class Y> struct map {
  struct iterator {
    friend struct map;
...
public:
  iterator begin() { return iterator(_Head._Ptr->_Left); }

  iterator end() { return iterator(_Head._Ptr); }
...
public:
  iterator begin() { return iterator(_Head._Ptr->_Right); }
